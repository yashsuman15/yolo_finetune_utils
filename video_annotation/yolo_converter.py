...
def convert_to_yolo_bbox(annotation_path, videos_dir, use_split=True, split_ratio=(0.7, 0.2, 0.1), output_dir="yolo_bbox_dataset"):
    ...
    This function reads a JSON file with bbox annotations, extracts the annotated
    frames from the corresponding videos, and saves them in YOLO detection format.
    It creates .txt label files and a data.yaml file for model training.
        ...
        for answer_section in item.get("latest_answer", []):
            ...
            for annotation in answer_section.get("answer", []):
                for frame_num_str, frame_data in annotation.get("frames", {}).items():
                    ...
                    # Convert to YOLO format: class_id x_center y_center width height (all normalized 0-1)
                    ...
                    frame_annotations[frame_num].append(f"{class_id} {x_center} {y_center} {bbox_width} {bbox_height}")
